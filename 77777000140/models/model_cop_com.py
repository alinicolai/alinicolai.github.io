...
class ModelCoupledCoPCoM():
    ...
    def get_ready_data(self, cop, com, frequency, com_acc=None, center=False):
        ...
        if com_acc is None:
            com_acc = compute_discrete_difference(com, order=2, dt=dt)
        ...
        com_spd = compute_discrete_difference(com, order=1, dt=dt)
        com = com[:n_min]
...
#        print([self.fit_com_results[i]["coefs"] for i in range(2)])
        ...
        com_spd[:start] = compute_discrete_difference(true_com, order=1, dt=dt)[:start]

        com_acc = np.zeros((int(len(true_cop)*length_factor),2))
        com_acc[:start-1] = compute_discrete_difference(true_com, order=2, dt=dt)[:start-1]
        
        cop_acc = np.zeros((int(len(true_cop)*length_factor),2))
        cop_acc[:start-1] = compute_discrete_difference(true_com, order=2, dt=dt)[:start-1]      
